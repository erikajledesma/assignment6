...
while not done:
    ...
    for i in range(90, 190, 20):
        '''drawing the bottom row of the left lights
        draws the line to separate the lights
        screen: pygame window
        GRAY: color of line
        list 1&2: the x and y allignemnt
        2: thickness of line '''
        pygame.draw.line(screen, GRAY, [110, 60], [210, 60], 2)
        ''' draws the bottom left row of lights
        screen: pygame window
        light_color: refers to function to see whether the lights will be yellow or grey
        list:(y movement,x movement,length of lightbulb, height of lightbulb) '''
    ...
    for i in range(90, 190, 20):
        ...
        '''draws the top left row of lights
        screen: pygame window
        light_color: refers to function to see whether the lights will be yellow or grey
        list:(y movement,x movement,length of lightbulb, height of lightbulb)'''
    ...
    for i in range(570, 670, 20):
        ...
        '''draws the top right row of lights
        screen: pygame window
        light_color: refers to function to see whether the lights will be yellow or grey
        list:(y movement,x movement,length of lightbulb, height of lightbulb)'''
        pygame.draw.ellipse(screen, light_color, [i + 20, 20, 20, 20])
    ...
    for i in range(570, 670, 20):
        ...
        '''draws the bottom right row of lights
        screen: pygame window
        light_color: refers to function to see whether the lights will be yellow or grey
        list:(y movement,x movement,length of lightbulb, height of lightbulb)'''
        pygame.draw.ellipse(screen, light_color, [i + 20, 40, 20, 20])
    ...
    list2 = [341, 200]
    num = 1
    drawLines1(screen, color, list1, list2, num)
    ...
    list2_2 = [324, 216]
    num2 = 1
    drawLines2(screen2, color2, list1_2, list2_2, num2)

    #net part 3
    def draw_net3(screen, color, num1, num2, width):
        ''' function that draws lines for right side of net
        screen: python screen display
        color: color of lines
        num1: x-coordinate of ending vector for line
        num2: y-coordinate of ending vector for line
        width: line thickness '''
        for n in range(9):
            pygame.draw.line(screen, color, [480,140], [num1, num2], width)
            num1 = num1 - 2
            num2 = num2 - 2
    ...
    def draw_net4(screen, color, num1, num2, width):
        ''' draws horizontal lines on the front side of the net
            screen: python display screen
            color: sets color of lines
            num1: y-coordinate of starting vector for line
            num2: y-coordinate of ending vector for line
            width: line thickness '''
        for n in range(10):
